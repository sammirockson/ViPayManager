...
extension SelectorContainerView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let business = self.itemsArray[indexPath.row]
        if let type = BusinessType.init(rawValue: indexPath.item) {
            self.delegate?.selectedBusiness(business: business, type: type)
            self.delegate?.dismissSelf()
        }
